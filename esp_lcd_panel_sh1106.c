...
static esp_err_t panel_sh1106_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    if (x_start >= x_end)
    {
        ...
        ESP_LOGE(TAG, "    x_start: %d, x_end: %d", x_start, x_end);
        return ESP_ERR_INVALID_ARG;
    }
    if (y_start >= y_end)
    {
        ESP_LOGE(TAG, "y_start must be less than y_end");
        ESP_LOGE(TAG, "    y_start: %d, y_end: %d", y_start, y_end);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    {
        ESP_LOGE(TAG, "the height of the bitmap must be a multiple of 8");
        return ESP_ERR_INVALID_ARG;
    }
    sh1106_panel_t *sh1106 = __containerof(panel, sh1106_panel_t, base);
    esp_lcd_panel_io_handle_t io = sh1106->io;

    const uint8_t sh1106_column_start_offset = 2;
    uint8_t start_column_index = x_start + sh1106_column_start_offset;
    uint8_t page_length = x_end - x_start;
    uint8_t start_page_index = y_start / 8;
    uint8_t end_page_index = y_end / 8;

    for (int y = start_page_index; y < end_page_index; y++) {
        uint8_t column_addr_low = start_column_index & 0x0F;
        uint8_t column_addr_high = start_column_index >> 4 & 0x0F;
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_color(io, -1, color_data + x_start + y * SH1106_WIDTH, page_length), TAG, "io tx color failed");
    }
